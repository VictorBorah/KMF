...
jQuery.validator.addMethod("validate_mobile", function(value, element) {
	...
	var jvalidate = $("#ClientFrm").validate({
		ignore: [],
		rules:
		{                           
			clientName: {required: true },								
			clientMobile: {required: true ,validate_mobile:true}												
		},
		messages:
		{	
			clientName:{required:"Enter Client name"},					
			clientMobile:{required:"Enter Mobile Number"}						
		}
...
$("#addClient").click(function(){
	...
	if($("#ClientFrm").valid())
	{
		...
		var form = $('#ClientFrm')[0];
		var main_formData = new FormData(form);
		
		 $.ajax({
				type: "POST",
				url: "Controllers/createClient",
				data: main_formData,
				processData: false,
				contentType: false,
				cache: false,
				...
				success: function (data) {
					...
					var str= "#dbdbea url('images/male.jpg')";
					$("#dpPreview").css("background", str);
					$("#dpPreview").css("background-size", "cover");
					$validator.resetForm();	
					$('#ClientFrm')[0].reset();	
					
					//$('#clientPlans').val([]);
					
					$("#clientPlans").select2( {}); // clear out values selected
					$("#clientPlans").select2({ allowClear: true }); // re-init to show default status
					
					$('.graph').waitMe('hide');
					showSuccesMsg("Client Added to System successfully");					
		...
		var main_formData = new FormData(form);
		 ...
		 $.ajax({
				...
				success: function (data) {
					...
					$("#editClient").attr("disabled", false);					
					$('.graph').waitMe('hide');
					showSuccesMsg("Client Updated successfully");					
...
function init_uploadModule()
{		
		
		$.uploadPreview({
			input_field: "#userDP",  
			preview_box: "#dpPreview",  
			label_field: ".image-label",   
			label_default: "Choose Photo",  
