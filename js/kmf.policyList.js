...
$('.datatable').DataTable({
	...
	*/
	...
	$(document).on("click", '.terminatePolicy', function(e){
		var rid=this.id;
		...
		callback: function (result) {
			if(result) terminate_Policy(dataID);
	...
	function terminate_Policy(k)
	{
		$('.graph').waitMe({effect : 'ios'});
		
		$.ajax({ 
			url: "Controllers/terminatePolicy",
			type:"GET",
			data: {pk:k},
			success: 
				function(data)
