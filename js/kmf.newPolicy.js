	...
	{       
	   ...
	   if(element.parent('.input-group').length) {
		...
		else			
		{
			...
			{
				 if (element.hasClass('kmf_select'))
				   {
...
if( $('#customerID').length )	$("#customerID").select2();	
...
$("#customerID").change(function(){
	...
	$.ajax({ 
				url: "Controllers/getCustomerProfile",
				type:"GET",
				data: {cid:$('#customerID').val()},
				success: 
					function(data)
					{		 
						$("#custBock").html("");
...
$("#planID").change(function(){
	...
	$.ajax({ 
				url: "Controllers/getPlanDetails",
				type:"GET",
				data: {pid:$('#planID').val()},
				success: 
					function(data)
...
var jvalidate = $("#newPolicyFrm").validate({
		ignore: [],
		rules:
		{                           
		...
		messages:
		{	
			customerID:{required:"Select Customer"},					
			planID:{required:"Select Plan"}					
...
$("#linkPolicy").click(function(){	
	...
	if($("#newPolicyFrm").valid())
	{
		$("#linkPolicy").attr("disabled", false);
		$('.graph').waitMe({effect : 'ios'});
		
		var form = $('#newPolicyFrm')[0];
		var main_formData = new FormData(form);
		
		 $.ajax({
				type: "POST",
				url: "Controllers/linkPolicy",
				data: main_formData,
				processData: false,
				contentType: false,
				cache: false,
				...
				success: function (data) {
					...
					{
						$("#linkPolicy").attr("disabled", false);	
						showSuccesMsg("Policy Linked to Customer successfully");
						location.reload();
						/*
						var $validator = $("#newPolicyFrm").validate();						
						$validator.resetForm();	
						$('#newPolicyFrm')[0].reset();						
						$("#customerID").select2( {}); // clear out values selected
						$("#customerID").select2({ allowClear: true }); // re-init to show default status					
						$("#planID").select2( {});
						$("#planID").select2({ allowClear: true });					
						$('.graph').waitMe('hide');
						$("#planBlock").html("");
						$("#custBock").html("");
						*/
