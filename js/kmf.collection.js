...
jQuery.validator.addMethod("validate_mobile", function(value, element) {
	...
	*/
	...
	$(document).on("click", '.collectBTN', function(e){
		var rid=this.id;
		var splitArr=rid.split("-");
		var dataID=splitArr[1];
		var dataName=splitArr[2];
		var planPK=splitArr[3];
		
		console.log("\n Record PK=>"+dataID);
		console.log("\n Record Title=>"+dataName);
		
		$('.page-container').waitMe({ effect : 'ios'	});
	    jsPanel.ziBase = 9000;
		collectionPane = jsPanel.create({
			id:'recordViewWin',		
			  theme: {
			...
			onwindowresize: true,
			headerTitle: 'Collect Cash',
			contentFetch:
			{
				resource: 'Controllers/collectionWindow?mapID='+dataID+'&planID='+planPK,
				beforeSend: function()
				{
				...
				done: function (panel, response) 
				{
					...
					//var itemName  =  $.trim($("#prodName1").val());
					collectionPane.setHeaderTitle("Collect Cash from: "+dataName);
	 ...
	 $(document).on("change", '.installmentDD', function(e){
		...
		var amt = $("#amount").val();
		var inst = $("#installmentsNum-"+dataID).val();
		
		
		
		var amt_Val = Math.round(amt * 100) / 100;
		var amt_Val = amt_Val.toFixed(2);
		
		var inst_Val = Math.round(inst * 100) / 100;
		var inst_Val = inst_Val.toFixed(2);
		 
	
		var  total_Amt = inst * amt;
		total_Amt = total_Amt.toFixed(2);
		
		showAlert("Total installment is "+total_Amt);
		$("#totAmount").val(total_Amt);
	...
	$(document).on("click", '.cashBtn', function(e){
		...
		var form_ID = "#cashForm-"+dataID;
		
		$(".cashBtn").attr("disabled", false);
		$('#recordViewWin').waitMe({effect : 'ios'});
		...
		var main_formData = new FormData(form);
		 ...
		 $.ajax({
				...
				success: function (data) {
					...
					$('#recordViewWin').waitMe('hide');
					closePanel(collectionPane);	
					showSuccesMsg("Cash Collected");					
