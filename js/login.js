...
var jvalidate = $("#loginFrm").validate({
	ignore: [],
	rules:
	{                           
...
$('#loginFrm').submit(function(e){	
	...
	{
		showFailureMsg("Invalid or Missing Credentials");
...
function processlogin()	
{
	...
	$('#lgnBx').waitMe({effect : 'ios'});
	
	$.ajax({
		type: "POST",
		url: "App/Controllers/validateUser",
		data: data,		
		processData: false,				
		contentType: false,
		cache: false,
		...
		success: function (data) {
			...
			console.log("User Validation Return: "+data);
			
			if(data==1)
			{																	
				window.location.href = "dashboard?b8osKo9LxY=0@giR19K4vUw=1";
				showAlert("All Okay ! \n Ready to Redirect!");					 
			}
			else
			{				
				showFailureMsg("Invalid Credentials !");
